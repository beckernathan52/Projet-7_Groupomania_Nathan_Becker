...
// Récupère la liste des Likes/Dislikes
const getAllLikes = async (req, res) => {
    try {
        const allLikes = await Like.findAll({include: Post})

        if (!allLikes.length) {
            return res.status(404).json({ error: "Aucun like n'a été trouvé."})
        }
...
export {likeOrDislike, getAllLikes}